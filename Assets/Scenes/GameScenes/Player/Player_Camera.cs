...
public class Player_Camera : MonoBehaviour
{
   ...
   //마우스 입력 값;
    ...
    void LateUpdate()                               //LateUpdate 에서 카메라 이동 문제 프레임 동기화 문제를 해결  
    {
        ...
        mosY = Mathf.Clamp(mosY, minYAngle, maxYAngle);             //상하 회전 제한 -35 ~ 60
        ...
        - (idealOffset * Distance)                  //플레이어 뒤로 Distance만큼
        + Vector3.up * Height;                      //위로 Height만큼
        ...
        if (Physics.Raycast(targetCenter, rayDirection, out hit, 
            rayDistance, collisionLayer, QueryTriggerInteraction.Ignore))
        {
            //충돌 시: 충돌 지점(hit.point)에서 뒤로 살짝 물러난 위치를 최종 위치로 설정
            finalCameraPosition = hit.point + hit.normal * collisionBuffer;     
            currentSmoothTime = 0.01f;              
        }
        else
        {
            //충돌 없을 시: 이상적인 위치를 최종 위치로 설정
            finalCameraPosition = idealPosition;        
        }


        // 충돌 여부에 따라 동적으로 변경된 currentSmoothTime 사용
        transform.position = Vector3.SmoothDamp(
            transform.position,                     //현재 위치
            finalCameraPosition,                    //목표 위치
            ref currentVelocity,                    // 속도 참조
            currentSmoothTime);                     // 도달 시간

        transform.LookAt(Target.position + Vector3.up * TargetVerticalOffset); 
    }
